---
layout: post
title:  "신입 백엔드 개발자, 임연지입니다."
date: 2021-03-25
desc: "신입 백엔드 개발자, 임연지입니다."
keywords: "Lyim, Java, Portfolio, Resume, About"
categories: [About]
tags: [Portfolio,Resume,About]
icon: icon-html
---



***


<br><br>
<!-- 프로필 사진 -->
<img style="display:flex; margin:auto; max-width:300px; border-radius:50%;" src="\static\assets\img\blog\resume\me.jpg"><br><br>
<!-- 간단 자기소개 -->
<center>
<p style="text-align:center; font-size:2rem; font-family: 'Spoqa Han Sans Neo', 'sans-serif';">안녕하세요!🌞<br><b>관찰과 이해</b>가 취미이고, <b>만들기</b>가 특기인<br></p> <h2><b style="color:rgb(0, 89, 255);">임연지</b> 입니다.</h2><br><br>

<div style="margin-bottom:3%;" >
    <code style="font-weight:200; font-family: 'Spoqa Han Sans Neo'; background-color:#7fa6ef; font-size:1.2em; color:white" class="language-plaintext highlighter-rouge"><b>관찰</b></code>&nbsp;&nbsp;
    <code style="font-weight:200; font-family: 'Spoqa Han Sans Neo'; background-color:#7fa6ef; font-size:1.2em; color:white" class="language-plaintext highlighter-rouge"><b>이해</b></code>&nbsp;&nbsp;
    <code style="font-weight:200; font-family: 'Spoqa Han Sans Neo'; background-color:#7fa6ef; font-size:1.2em; color:white" class="language-plaintext highlighter-rouge"><b>호기심</b></code>&nbsp;&nbsp;
    <code style="font-weight:200; font-family: 'Spoqa Han Sans Neo'; background-color:#7fa6ef; font-size:1.2em; color:white" class="language-plaintext highlighter-rouge"><b>신기술</b></code><br><br>
</div>
<div style="position:static;">
    <a href="#intro" style="text-decoration:underline; color:#7fa6ef; font-family: 'Spoqa Han Sans Neo'; font-size:1.5em;">Introduce</a>
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    <a style="text-decoration:underline; color:#7fa6ef; font-family: 'Spoqa Han Sans Neo'; font-size:1.5em;" href="#stack">Stack</a>
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    <a style="text-decoration:underline; color:#7fa6ef;font-family: 'Spoqa Han Sans Neo'; font-size:1.5em;" href="#edu">Education</a>
</div><br>
<!-- About -->
<hr style="margin-bottom:10%;">
<!-- Introduce -->
<p id="intro"><code style="font-family: 'Spoqa Han Sans Neo'; background-color:rgb(0, 89, 255); font-size:3rem; color:white" class="language-plaintext highlighter-rouge"><b>&nbsp; Introduce &nbsp;</b></code> </p><br><br>
<!-- desc 1 -->
<img style="width:30%; margin-bottom:6%;" src="\static\assets\img\blog\resume\basic.jpg">
<h2 style="color:rgb(0, 89, 255); font-weight:normal">" <b>기본 원리</b>의 이해를 중요시 합니다. "</h2><br>
<p style="margin-bottom:10%;">뿌리가 깊어야 건강하게 성장할 수 있듯, 기본을 이해할 수 있어야 응용도 가능하다고 생각합니다.<br>기초를 조금이라도 더 다지기 위해 학원에서도 끝까지 학생들과 함께 스터디를 해왔습니다.<br>앞으로도 꾸준하게 발전해서 다양한 변수에도 흔들리지 않는 단단한 결과물을 만들어 내고 싶습니다.</p>
<!-- desc 2 -->
<img style="max-width:70%;" src="\static\assets\img\blog\resume\create.jpg">
<h2 style="color:rgb(0, 89, 255); font-weight:normal">" 다양한 <b>창작</b> 경험이 있습니다. "</h2><br>
<p>살아오면서 다양한 종류의 창작 활동을 꾸준히 해왔습니다. <br> 지금도 새로운 도전과 창작활동에 마음이 설레입니다. <br> 무언가를 만들어 내는 것은 제가 가장 즐기는 일입니다.</p><br>
<ul style="margin-bottom:10%;">
    <li style="color:#7fa6ef">다수의 창작 및 전시회 활동 경험</li>
    <li style="color:#7fa6ef">문화행사 기획 및 섭외 경험</li>
</ul>
<!-- desc 3 -->
<img style="max-width:70%; margin-bottom:5%;" src="\static\assets\img\blog\resume\ac.jpg">
<h2 style="color:rgb(0, 89, 255); font-weight:normal">" <b>소통</b>과 <b>협업</b>에 강점이 있습니다. "</h2><br>
<p>다양한 연령대, 다른 성향을 가진 많은 사람들과 협력하여 자주 활동했습니다. <br>이러한 경험들을 바탕으로 타인과의 소통 능력을 향상시킬 수 있었고,<br>총 세번의 프로젝트에서 모두 팀장을 맡아 진행하며 한 명의 탈주도 없이 모든 프로젝트를 무사히 마쳤습니다.</p><br>
<ul style="margin-bottom:10%;">
    <li style="color:#7fa6ef">비영리 청년문화단체 활동, 지역 문화 협동조합 회원</li>
    <li style="color:#7fa6ef">문화체육관광부/ 시시콜콜 비영리단체 지원사업 2회 선정</li>
    <li style="color:#7fa6ef">행정안전부/ 청년 공동체 활성화 사업 경진대회 공동체상 수상</li>
</ul>
<!-- Introduce -->
<hr style="margin-bottom:5%;">
<!-- Stack -->
<br><br><p id="stack" style="margin-bottom:7%;"><code style="font-family: 'Spoqa Han Sans Neo'; background-color:rgb(0, 89, 255); font-size:3rem; color:white" class="language-plaintext highlighter-rouge"><b>&nbsp; Stack &nbsp;</b></code></p>
<!-- java -->
<h2 style="color:rgb(0, 89, 255)"><b># Java</b></h2>
<p style="margin-bottom:2%;">: 가장 자신있게 사용할 수 있으며 거의 모든 프로젝트에서 자바 언어를 사용했습니다.</p>
<!-- js -->
<h2 style="color:rgb(0, 89, 255)"><b># JavaScript</b></h2>
<p style="margin-bottom:2%;">: 유효성 검사, 라이브러리를 프로젝트에 적용하고 있으며 조금 더 깔끔한 코드를 짜기 위해 학습 중 입니다.</p>
<!-- oracle -->
<h2 style="color:rgb(0, 89, 255)"><b># Oracle</b></h2>
<p style="margin-bottom:2%;">: 데이터 CRUD가 가능하며, 모든 웹 프로젝트에서 Oracle DB를 사용했습니다.</p>
<!-- HTML / CSS -->
<h2 style="color:rgb(0, 89, 255)"><b># HTML / CSS</b></h2>
<p style="margin-bottom:2%;">: 원하는 UI 를 만들고 프로젝트에 적용시킬 수 있습니다.</p><br>
<!-- experienced -->
<h2 style="color:#555555"><b># Experienced</b></h2>
<p style="margin-bottom:2%; color:#aaaaaa;">Python</p><br>
<!-- tools -->
<h2 style="color:#555555"><b># Tools</b></h2>
<p style="margin-bottom:2%;color:#a5c4ff;">Spring Tool Suite 3, Eclipse, Visual Studio Code</p>
<p style="margin-bottom:2%;color:#aaaaaa;">Sql plus, Sql Developer, eXERD</p>
<p style="margin-bottom:2%;color:#a5c4ff;">Ajax, Tomcat9, Web Socket</p>
<p style="margin-bottom:2%;color:#aaaaaa;">Bootstrap4, Semantic UI, Kakao API, SweetAlert2, Simple Data Picker, Toastr</p>
<p style="margin-bottom:2%;color:#a5c4ff;">Slack, Github</p>
<!-- Stack -->
<hr style="margin-bottom:10%;">
<!-- Education -->
<p id="edu" style="margin-bottom:10%;"><code style="font-family: 'Spoqa Han Sans Neo'; background-color:rgb(0, 89, 255); font-size:3rem; color:white" class="language-plaintext highlighter-rouge"><b>&nbsp; Education &nbsp;</b></code></p>

<h2 style="font-weight:400; color:rgb(0, 89, 255)"><b># 비트캠프</b> 신촌센터 181기</h2>
<p style="margin-bottom:2%;">2020.10. ~ 2021. 3.   /    
<a href="https://www.bitcamp.co.kr/class/54?sca=%EA%B5%AD%EB%B9%84%EC%A7%80%EC%9B%90%EA%B3%BC%EC%A0%95&brc=%EC%8B%A0%EC%B4%8C%EC%84%BC%ED%84%B0">교육과정 보기</a></p>
<!-- 태그 -->
<div style="margin-bottom:3%;">
    <code style="background-color:#a5c4ff; font-size:1.3em; color:white" class="language-plaintext highlighter-rouge"><b>&nbsp; # 모범상 &nbsp;</b></code>&nbsp;&nbsp;
    <code style="background-color:#a5c4ff; font-size:1.3em; color:white" class="language-plaintext highlighter-rouge"><b>&nbsp; # 스터디장 &nbsp;</b></code>
</div>
<!-- 내용 -->
<p>비트캠프 신촌센터 181기 교육 과정을 수료하였고,<br>웹 개발을 위한 기초 개념을 학습했습니다.</p>
<ul style="margin-bottom:20%;">
    <li style="color:#7fa6ef">백엔드 개발 기초 개념 학습</li>
    <li style="color:#7fa6ef">프론트엔드 기초 개념 학습</li>
    <li style="color:#7fa6ef">개인 프로젝트 : 1회</li>
    <li style="color:#7fa6ef">팀 프로젝트 : 3회</li>
</ul>